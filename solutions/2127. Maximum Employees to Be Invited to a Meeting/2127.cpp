...
class Solution {
 public:
  int maximumInvitations(vector<int>& favorite) {
    ...
    for (int i = 0; i < n; ++i)
      if (!seen[i])
        dfs(graph, i, parent, seen, state, maxCycleLength);

    return max(sumComponentsLength / 2, maxCycleLength);
  }

 private:
  void dfs(const vector<vector<int>>& graph, int u, vector<int>& parent,
           vector<bool>& seen, vector<State>& state, int& maxCycleLength) {
    seen[u] = true;
    state[u] = VISITING;
    ...
    for (const int v : graph[u]) {
      if (!seen[v]) {
        parent[v] = u;
        dfs(graph, v, parent, seen, state, maxCycleLength);
